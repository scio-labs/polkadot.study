...
## Adding new function to the contract
...
## Connect UI to the updated contract

As usual, we will need to connect the frontend to the newly deployed contract. However, in this case we also need to add new UI into the dApp so that we can execute `reverse_message` from the frontend.
...
That's amazing, isn't it? We have successfully added a new feature to the dApp. 

Now, let's move on to the next challenge. We obviously want everyone on the internet to be able to use our dApp.
In order to do that, we will need to deploy the frontend through a hosting platform.
We will show how to do the deployment on two different hosting services: Vercel and Fly.io, and see how it works differently on each platform.