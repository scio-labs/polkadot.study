...
## Deploy contract on a local node
...
:::info
Alternatively, you can also use Contracts-UI to deploy contracts manually with the browser.
:::

## Connect frontend to the deployed contract

We are done with the contract stuff. Now let’s connect the frontend to recently deployed contract. First, go to the frontend directory with `cd ../frontend`.
...
Congratulations, you have made it this far! The following parts will be more about explanations on the codebase and how it works. We will start on `Project Root`, then `Frontend`, and we will end it on `Contracts`.